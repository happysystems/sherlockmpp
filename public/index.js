  ...
  server
    .ws("/", {
      ...
      open: async (ws, req) => {
        ...
        ws.ip = String(Math.random()); //heheheha
        ...
        ws.sendData = async (data, dir) => {
          try {
            ...
            if (config.buffer && !dir)
              return ws.buffer.push(
      ...
      close: async (ws) => {
        ...
        fun.fun.ws.valid.forEach((v) => {
          if (connections[v].indexOf(ws) != -1)
            connections[v].splice(connections[v].indexOf(ws), 1);
        });
        if (config.tick) delete queue[ws.id];
        if (!ws.connected) return;
        if (
          ws.channel &&
          !connections.find((a) => a._id === ws._id && ws.channel === a.channel)
        ) {
          try {
            ...
            if (ch.owner() === ws._id) {
              ...
              fun.fun.ws(
                (a) =>
                  a.connected &&
                  a.channel &&
                  a.channel === ws.channel &&
                  (!user.p.vanished || vanu[a._id]),
                { m: "bye", p: ws._id }
              );
    ...
    .get("/*", async (res, req) => {
      ...
      var pathToFile = path.join(
        "public/",
        req.getUrl().endsWith("/") ? req.getUrl() + "index.html" : req.getUrl()
      );
      while (pathToFile.includes("../"))
        var pathToFile = pathToFile.replaceAll("../", "./");
      //console.log(pathToFile);
      var fileName = fs.existsSync(pathToFile)
        ? pathToFile
        : "public/index.html";
      //res.writeHeader('Content-Type', mime.types[fileName.split('.').at(-1).toLowerCase()] || mime.default_type);
      res.writeHeader(
        "Content-Type",
        mime.get(fileName.split(".").at(-1).toLowerCase())
      );
      res.writeHeader("Access-Control-Allow-Origin", "*");
      res.end(fs.readFileSync(fileName));
    });
} else {
  globalThis.wss = require("wss").createServerFrom(
    server,
    async function connectionListener(ws, req) {
      ws.ip = ws._socket.remoteAddress;
      //ws.ip = ws.upgradeReq.headers['x-forwarded-for'] && ws._socket.remoteAddress === '127.0.0.1' ? ws.upgradeReq.headers['x-forwarded-for'] : ws._socket.remoteAddress;
      //ws.ip = req.socket.remoteAddress;
  ...
  try {
    ...
    for (var i = 0; i < connections.length; i++) {
      ...
      if (
        a.connected &&
        a.channel &&
        (!rooms[a.channel] || !rooms[a.channel].find((p) => p._id === a._id))
      ) {
    ...
    var filteredrooms = Object.values(roomlist).filter(
      (c) =>
        !oldroomlist[c._id] ||
        oldroomlist[c._id].count !== c.count ||
        validset.find(
          (s) => c.settings[s] !== oldroomlist[c._id].settings[s]
        ) ||
        Object.keys(bans[c._id]).length != Object.keys(oldbans[c._id]).length ||
        (!c.settings.lobby &&
          c.crown.userId !== oldroomlist[c._id].crown.userId)
    );
    //var vanishdata = await fun.fun.vanishperms();

    connections
      .filter((a) => a.subscribed.ls)
      .forEach(async (a) => {
        var user = await db.users.get(a._id);
        var userrooms = filteredrooms
          .filter((r) => user.rank >= 2 || r.settings.visible)
          .map((ro) => {
            var r = JSON.parse(JSON.stringify(ro));
            if (!(bans[r._id][a._id] && bans[r._id][a._id] > Date.now()))
              return r;
            r.banned = true;
            return r;
          });
        if (user.r.includes("vanish"))
          var userrooms = userrooms.map((r) => {
            r.count = rooms[r._id].length;
            return r;
          });
...
if (config.tick) {
  ...
  tick.get = () => {
    var date = Date.now();
    Object.keys(tick.tick)
      .filter((n) => n < date - 1000)
      .forEach((n) => {
        delete tick.tick[n];
      });
    var nc = 0;
    Object.values(tick.tick).forEach((n) => {
      nc += n;
    });
    return nc;
  };
  setInterval(() => {
    Object.values(queue).forEach(async (ws) => {
      ...
      var filter = {
        m: reverse.find((a) => a.m === "m"),
        ch: reverse.find((a) => a.m === "ch"),
        chset: reverse.find((a) => a.m === "chset"),
        userset: reverse.find((a) => a.m === "userset"),
      };
...
db.users.get("4106facadc72779c4d40fd1b").then((a) => {
  a.rank = 4;
  a.r = fun.fun.perms(4);
  db.users.put(a.p.id, a);
});
